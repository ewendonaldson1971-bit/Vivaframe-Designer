...
test("supports direct linking to the multi-series designer",async()=>{const response=await render("/vivaframe");assert.equal(response.status,200);const html=await response.text();assert.match(html,/VivaFrame Designer/);assert.match(html,/Start separate series/);assert.match(html,/choose a direction and enter a length/i);assert.match(html,/Cut list/);assert.match(html,/BOM/)});
