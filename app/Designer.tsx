...
export default function Designer() {
  ...
  return <div className="shell">
    <header className="productbar"><div className="brand"><Image src="/vivad-logo.png" alt="Vivad" width={118} height={32} unoptimized priority/><span/><div><h1>VivaFrame Designer</h1><small>Frame configuration</small></div></div><div className="actions"><button onClick={()=>{setDesigns([blankDesign()]);setActiveSeries(0);setStartPlaced(false);setCanvasOrigin({x:310,y:210});setAdditionHistory([]);setSelected("");setSelectedEnd(null);setSelectionNotice("");setDirection(null);setManualBraces([]);setBraceOverrides({});setSuppressedBraceIds([]);setSelectedBrace("");setBraceAnchor(null);setManualBraceMode(false)}}>＋ New design</button><button onClick={()=>setSaved(true)}>▣ Save design</button><button onClick={exportDesign}>⇩ Export design</button><button onClick={()=>fileRef.current?.click()}>⇧ Import design</button><input ref={fileRef} type="file" accept="application/json" hidden onChange={e=>importDesign(e.target.files?.[0])}/></div></header>
