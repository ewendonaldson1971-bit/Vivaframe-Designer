...
import { addBoundarySegment, braceHasSideConnection, createManualBrace, crossBracePiecesForDesigns, deleteBraceAndExtend, Design, Heading, historyFor, pointsFor, prepareFrameTakeoffForDesigns, profileFacingSides, removeBoundarySegment, splitForStock, summaryForDesigns, type BraceConfig, type BracePiece, type Point } from "../lib/geometry";
import { downloadFrameDrawingPdf } from "../lib/drawing-pdf";
...
export default function Designer() {
  ...
  const [automaticBraces, setAutomaticBraces] = useState(true);
  const [manualBraces, setManualBraces] = useState<BracePiece[]>([]);
  const [braceOverrides, setBraceOverrides] = useState<Record<string,"h-brace"|"mini-brace">>({});
  const [suppressedBraceIds, setSuppressedBraceIds] = useState<string[]>([]);
  const [selectedBrace, setSelectedBrace] = useState("");
  const [manualBraceMode, setManualBraceMode] = useState(false);
  const [braceOrientation, setBraceOrientation] = useState<"horizontal"|"vertical">("vertical");
  const [manualBraceKind, setManualBraceKind] = useState<"h-brace"|"mini-brace">("mini-brace");
  const [manualBraceOffset, setManualBraceOffset] = useState(0);
  const [braceAnchor, setBraceAnchor] = useState<{seriesIndex:number;pointIndex:number;point:Point}|null>(null);
  const [braceNotice, setBraceNotice] = useState("");
  ...
  const seriesProfileSides = useMemo(() => profileFacingSides(designs), [designs]);
  ...
  const automaticBracePieces = useMemo(()=>automaticBraces?crossBracePiecesForDesigns(designs,braceConfig).filter(piece=>!suppressedBraceIds.includes(piece.id)):[],[automaticBraces,designs,braceConfig,suppressedBraceIds]);
  const overriddenBracePieces = useMemo(()=>[...automaticBracePieces,...manualBraces].map(piece=>{const kind=braceOverrides[piece.id]??piece.kind;return {...piece,kind,widthMm:kind==="h-brace"?braceConfig.hBraceWidth:braceConfig.miniBraceWidth}}),[automaticBracePieces,manualBraces,braceOverrides,braceConfig]);
  const bracePieces = useMemo(()=>overriddenBracePieces.map(piece=>braceHasSideConnection(piece,overriddenBracePieces)&&piece.kind!=="h-brace"?{...piece,kind:"h-brace" as const,widthMm:braceConfig.hBraceWidth}:piece),[overriddenBracePieces,braceConfig.hBraceWidth]);
  const bracePiecesBySeries = useMemo(()=>[bracePieces],[bracePieces]);
  const braceDrawings = useMemo(()=>[...bracePieces].sort((a,b)=>Number(a.kind==="h-brace")-Number(b.kind==="h-brace")),[bracePieces]);
  const selectedBracePiece = bracePieces.find(piece=>piece.id===selectedBrace)??null;
  const selectedBraceHasSideConnection = selectedBracePiece?braceHasSideConnection(selectedBracePiece,bracePieces):false;
  const takeoff = useMemo(() => ({...prepareFrameTakeoffForDesigns(designs, profile, "configured",quantity),bracePieces:bracePieces.map(piece=>({kind:piece.kind,orientation:piece.orientation,lengthMm:piece.lengthMm}))}), [designs, profile, quantity, bracePieces]);
  ...
  function selectBraceNode(seriesIndex:number,pointIndex:number,point:Point){setBraceAnchor({seriesIndex,pointIndex,point});setBraceNotice(`Node ${pointIndex+1} of series ${seriesIndex+1} selected as the offset datum.`);setSelectedBrace("")}
  function selectBracePiece(id:string){setSelectedBrace(id);setSelected("");setSelectedEnd(null);setBraceNotice("")}
  function changeSelectedBraceKind(kind:"h-brace"|"mini-brace"){
    const piece=bracePieces.find(item=>item.id===selectedBrace);if(!piece)return;
    if(kind==="mini-brace"&&braceHasSideConnection(piece,bracePieces)){setBraceNotice("This brace receives a side connection and must remain an H Brace.");return}
    setBraceOverrides(current=>({...current,[piece.id]:kind}));setBraceNotice(`Brace changed to ${kind==="h-brace"?"H Brace":"Mini Brace"}.`);
  }
  function deleteSelectedBrace(){
    if(!selectedBrace)return;const result=deleteBraceAndExtend(designs,bracePieces,selectedBrace,braceConfig,`extended-${Date.now()}`);if(!result.removedIds.length)return;
    setSuppressedBraceIds(current=>[...new Set([...current,...result.removedIds])]);setManualBraces(current=>[...current.filter(piece=>!result.removedIds.includes(piece.id)),...result.replacements]);setBraceOverrides(current=>{const next={...current};result.removedIds.forEach(id=>delete next[id]);result.promotedIds.forEach(id=>next[id]="h-brace");return next});setSelectedBrace(result.replacements[0]?.id??"");setBraceNotice(result.replacements.length?`Brace deleted. ${result.replacements.length} connected ${result.replacements.length===1?"brace was":"braces were"} extended to the next support.`:"Brace deleted.");
  }
  ...
  function canvasDesignPoint(e:ReactMouseEvent<SVGSVGElement>){const svg=e.currentTarget,screen=svg.createSVGPoint();screen.x=e.clientX;screen.y=e.clientY;const local=screen.matrixTransform(svg.getScreenCTM()!.inverse());return {x:designs[0].start.x+(local.x-canvasOrigin.x)/sx,y:designs[0].start.y+(local.y-canvasOrigin.y)/sx}}
  function handleCanvasClick(e:ReactMouseEvent<SVGSVGElement>){
    if(!manualBraceMode){placeStart(e);return}
    if(!braceAnchor){setBraceNotice("Select a frame node first, then click the bay where the brace should be fitted.");return}
    const result=createManualBrace(designs,bracePieces,braceAnchor.point,braceOrientation,manualBraceOffset,canvasDesignPoint(e),manualBraceKind,braceConfig,`manual-${Date.now()}`);
    if(!result){setBraceNotice("No pair of parallel extrusions encloses that point at the specified offset.");return}
    setManualBraces(current=>[...current,result.piece]);setBraceOverrides(current=>({...current,...Object.fromEntries(result.intersectedBraceIds.map(id=>[id,"h-brace" as const]))}));setSelectedBrace(result.piece.id);setBraceNotice(`Manual ${manualBraceKind==="h-brace"?"H Brace":"Mini Brace"} added at ${manualBraceOffset.toLocaleString()} mm from the selected node.`);
  }
  function addSeries(){
  ...
  useEffect(()=>{ const key=(e:KeyboardEvent)=>{const target=e.target as HTMLElement;if(["INPUT","SELECT","TEXTAREA"].includes(target.tagName))return;if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==="z"){e.preventDefault();undo()}else if(e.key==="Delete"){e.preventDefault();if(selectedBrace)deleteSelectedBrace();else deleteSelection()}else if(e.key==="ArrowLeft")setDirection("W");else if(e.key==="ArrowUp")setDirection("N");else if(e.key==="ArrowRight")setDirection("E");else if(e.key==="ArrowDown")setDirection("S");else if(e.key==="Enter")addSegment();}; window.addEventListener("keydown",key); return()=>window.removeEventListener("keydown",key); });
  function exportDesign(){ const a=document.createElement("a"),blob=new Blob([JSON.stringify({schemaVersion:5,name:"Reception display frame",designs,bracing:{automatic:automaticBraces,manual:manualBraces,overrides:braceOverrides,suppressed:suppressedBraceIds},productIds:{profile}},null,2)],{type:"application/json"});a.href=URL.createObjectURL(blob);a.download="vivaframe-design.json";a.click();URL.revokeObjectURL(a.href); }
  function downloadPdf(){downloadFrameDrawingPdf({designs,braces:bracePieces,title:"Reception display frame",profileLabel,profileHeightMm:selectedExtrusion?.source?.height??40,quantity})}
  async function importDesign(file?:File){if(!file)return;const data=JSON.parse(await file.text()),imported:Design[]=data.designs??[data.design];setDesigns(imported);setAutomaticBraces(data.bracing?.automatic??true);setManualBraces(data.bracing?.manual??[]);setBraceOverrides(data.bracing?.overrides??{});setSuppressedBraceIds(data.bracing?.suppressed??[]);setActiveSeries(0);setStartPlaced(true);setCanvasOrigin({x:310,y:210});setAdditionHistory([]);setSelectedEnd("end");setSelectedBrace("");setBraceAnchor(null);setProfile(data.productIds?.profile||"vivaframe-ss25");fitDesigns(imported);}

  return <div className="shell">
    <header className="productbar"><div className="brand"><Image src="/vivad-logo.png" alt="Vivad" width={118} height={32} unoptimized priority/><span/><div><h1>VivaFrame Designer</h1><small>Frame configuration</small></div></div><div className="actions"><button onClick={()=>{setDesigns([blankDesign()]);setActiveSeries(0);setStartPlaced(false);setCanvasOrigin({x:310,y:210});setAdditionHistory([]);setSelected("");setSelectedEnd(null);setSelectionNotice("");setDirection(null);setManualBraces([]);setBraceOverrides({});setSuppressedBraceIds([]);setSelectedBrace("");setBraceAnchor(null);setManualBraceMode(false)}}>＋ New design</button><button onClick={()=>setSaved(true)}>▣ Save design</button><button onClick={downloadPdf} disabled={!stats.closed}>⇩ Download A3 PDF</button><button onClick={exportDesign}>⇩ Export design</button><button onClick={()=>fileRef.current?.click()}>⇧ Import design</button><input ref={fileRef} type="file" accept="application/json" hidden onChange={e=>importDesign(e.target.files?.[0])}/></div></header>
    ...
    <main className="layout">
      <aside className="config">
        ...
        <Section title="Cross braces"><label className="brace-auto"><span>Automatic brace assignment</span><input type="checkbox" checked={automaticBraces} onChange={e=>setAutomaticBraces(e.target.checked)}/></label><button className={`new-series ${manualBraceMode?"brace-mode-active":""}`} onClick={()=>{setManualBraceMode(value=>!value);setBraceNotice("")}} disabled={!stats.closed}>{manualBraceMode?"✓ Manual brace mode":"＋ Add brace manually"}</button>{manualBraceMode&&<div className="manual-brace-controls"><p><b>1.</b> Select any frame node as the datum.</p><div className="brace-choice"><button className={braceOrientation==="horizontal"?"chosen":""} onClick={()=>setBraceOrientation("horizontal")}>Horizontal</button><button className={braceOrientation==="vertical"?"chosen":""} onClick={()=>setBraceOrientation("vertical")}>Vertical</button></div><label>Offset from selected node<div className="brace-offset"><input type="number" step={snap?50:1} value={manualBraceOffset} onChange={e=>setManualBraceOffset(Number(e.target.value)||0)}/><span>mm</span></div></label><div className="brace-choice"><button className={manualBraceKind==="mini-brace"?"chosen":""} onClick={()=>setManualBraceKind("mini-brace")}>Mini Brace</button><button className={manualBraceKind==="h-brace"?"chosen":""} onClick={()=>setManualBraceKind("h-brace")}>H Brace</button></div><p><b>2.</b> Click the bay between the two extrusions to connect.</p><small>{braceAnchor?`Datum: series ${braceAnchor.seriesIndex+1}, node ${braceAnchor.pointIndex+1} (${braceAnchor.point.x}, ${braceAnchor.point.y}) mm`:"No datum node selected."}</small></div>}{selectedBracePiece&&<div className="selected-brace-controls"><b>Selected: {selectedBracePiece.kind==="h-brace"?"H Brace":"Mini Brace"}</b><small>{selectedBracePiece.lengthMm.toLocaleString()} mm · {selectedBracePiece.orientation}</small><div className="brace-choice"><button className={selectedBracePiece.kind==="mini-brace"?"chosen":""} onClick={()=>changeSelectedBraceKind("mini-brace")} disabled={selectedBraceHasSideConnection}>Mini Brace</button><button className={selectedBracePiece.kind==="h-brace"?"chosen":""} onClick={()=>changeSelectedBraceKind("h-brace")}>H Brace</button></div>{selectedBraceHasSideConnection&&<small>Locked as H Brace because another brace connects to its side.</small>}<button className="remove-brace" onClick={deleteSelectedBrace}>Delete selected brace</button></div>}{braceNotice&&<p className="brace-notice">{braceNotice}</p>}</Section>
      </aside>
      <section className="stage">
        ...
        <div className={`canvas ${snap?"grid":""}`}>
          <svg viewBox="0 0 620 420" className={`${!startPlaced?"placing":""} ${manualBraceMode?"manual-brace-mode":""}`} onClick={handleCanvasClick} role={startPlaced?"img":"button"} tabIndex={!startPlaced?0:undefined} aria-label={startPlaced?`${designs.length} series frame ${stats.width} by ${stats.height} millimetres`:"Click a grid point to place the starting point"}>
            {startPlaced&&<>
            {braceDrawings.map(piece=><BraceDrawing key={piece.id} piece={piece} xy={xy} frameProfileHeight={Math.max(selectedExtrusion?.source?.width??0,selectedExtrusion?.source?.height??0)} selected={selectedBrace===piece.id} onSelect={()=>selectBracePiece(piece.id)}/>)}
            {designs.map((series,seriesIndex)=>seriesIndex===activeSeries?null:<g key={`series-${seriesIndex}`} className="inactive-series">{series.segments.map((segment,index)=>{const seriesPath=seriesPoints[seriesIndex],a=xy(seriesPath[index]),b=xy(seriesPath[index+1]),length=Math.hypot(b.x-a.x,b.y-a.y)||1,side=seriesProfileSides[seriesIndex],ox=-(b.y-a.y)/length*side,oy=(b.x-a.x)/length*side;return <g key={segment.id} onClick={e=>{e.stopPropagation();selectSeriesSegment(seriesIndex,segment.id)}}><line className="hit" x1={a.x} y1={a.y} x2={b.x} y2={b.y}/><line className="rail" x1={a.x} y1={a.y} x2={b.x} y2={b.y}/><line className="face" x1={a.x} y1={a.y} x2={b.x} y2={b.y}/><line className="profile-highlight" x1={a.x+ox*5} y1={a.y+oy*5} x2={b.x+ox*5} y2={b.y+oy*5}/><line className="profile-groove" x1={a.x-ox*3} y1={a.y-oy*3} x2={b.x-ox*3} y2={b.y-oy*3}/></g>})}<circle className="series-point" cx={xy(series.start).x} cy={xy(series.start).y} r="5"/></g>)}
            ...
            {manualBraceMode&&seriesPoints.flatMap((seriesPath,seriesIndex)=>seriesPath.slice(0,-1).map((point,pointIndex)=>{const q=xy(point),active=braceAnchor?.seriesIndex===seriesIndex&&braceAnchor.pointIndex===pointIndex;return <g key={`brace-node-${seriesIndex}-${pointIndex}`} className={`brace-node ${active?"brace-node-selected":""}`} onClick={e=>{e.stopPropagation();selectBraceNode(seriesIndex,pointIndex,point)}} role="button" tabIndex={0} aria-label={`Use series ${seriesIndex+1} node ${pointIndex+1} as brace datum`}><circle className="brace-node-hit" cx={q.x} cy={q.y} r="12"/><circle className="brace-node-dot" cx={q.x} cy={q.y} r="5"/></g>}))}
            </>}
          </svg>
...
function BraceDrawing({piece,xy,frameProfileHeight,selected,onSelect}:{piece:BracePiece;xy:(point:{x:number;y:number})=>CanvasPoint;frameProfileHeight:number;selected:boolean;onSelect:()=>void}){
  ...
  return <g className={`cross-brace ${piece.kind} ${selected?"brace-selected":""}`} onClick={e=>{e.stopPropagation();onSelect()}} role="button" tabIndex={0} onKeyDown={e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();onSelect()}}} aria-label={`${piece.kind==="h-brace"?"H Brace":"Mini Brace"} ${piece.lengthMm} millimetres`}><line className="brace-hit" x1={start.x} y1={start.y} x2={end.x} y2={end.y}/><line x1={start.x} y1={start.y} x2={end.x} y2={end.y} style={{strokeWidth}}/><circle cx={start.x} cy={start.y} r="2.5"/><circle cx={end.x} cy={end.y} r="2.5"/></g>;
}
