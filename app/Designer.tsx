...
import { addBoundarySegment, crossBracePiecesForDesigns, Design, Heading, historyFor, pointsFor, prepareFrameTakeoffForDesigns, profileFacingSides, removeBoundarySegment, splitForStock, summaryForDesigns, type BraceConfig, type BracePiece } from "../lib/geometry";
...
export default function Designer() {
  ...
  const bracePieces = useMemo(()=>crossBracePiecesForDesigns(designs,braceConfig),[designs,braceConfig]);
      ...
      <section className="stage">
        ...
        <div className={`canvas ${snap?"grid":""}`}>
          <svg viewBox="0 0 620 420" className={!startPlaced?"placing":""} onClick={placeStart} role={startPlaced?"img":"button"} tabIndex={!startPlaced?0:undefined} aria-label={startPlaced?`${designs.length} series frame ${stats.width} by ${stats.height} millimetres`:"Click a grid point to place the starting point"}>
            {startPlaced&&<>
            {braceDrawings.map(piece=><BraceDrawing key={piece.id} piece={piece} xy={xy} frameProfileHeight={Math.max(selectedExtrusion?.source?.width??0,selectedExtrusion?.source?.height??0)}/>)}
