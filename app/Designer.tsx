      ...
      <section className="stage">
        ...
        <div className={`canvas ${snap?"grid":""}`}>
          <svg viewBox="0 0 620 420" className={!startPlaced?"placing":""} onClick={placeStart} role={startPlaced?"img":"button"} tabIndex={!startPlaced?0:undefined} aria-label={startPlaced?`${designs.length} series frame ${stats.width} by ${stats.height} millimetres`:"Click a grid point to place the starting point"}>
            ...
            {bracePiecesBySeries[activeSeries].map(piece=><BraceDrawing key={piece.id} piece={piece} xy={xy} scale={sx}/>)}
...
function BraceDrawing({piece,xy,scale}:{piece:BracePiece;xy:(point:{x:number;y:number})=>CanvasPoint;scale:number}){
  const start=xy(piece.start),end=xy(piece.end),strokeWidth=Math.max(2,piece.widthMm*scale);
...
function Bom({takeoff,profileLabel}:{takeoff:ReturnType<typeof prepareFrameTakeoffForDesigns>;profileLabel:string}){const hBraces=takeoff.bracePieces.filter(piece=>piece.kind==="h-brace"),miniBraces=takeoff.bracePieces.filter(piece=>piece.kind==="mini-brace"),locks=takeoff.bracePieces.length*2;return <div className="data"><h3>Bill of materials</h3><p>Calculated directly from all segment series.</p><article><span>{takeoff.cutPieces.length}</span><div><b>{profileLabel}</b><small>{takeoff.profileId}</small></div><em className="bom-status">Required</em></article>{hBraces.length>0&&<article><span>{hBraces.length}</span><div><b>H Brace</b><small>{hBraces.reduce((sum,piece)=>sum+piece.lengthMm,0).toLocaleString()} mm total</small></div><em className="bom-status">Required</em></article>}{miniBraces.length>0&&<article><span>{miniBraces.length}</span><div><b>Mini Brace</b><small>{miniBraces.reduce((sum,piece)=>sum+piece.lengthMm,0).toLocaleString()} mm total</small></div><em className="bom-status">Required</em></article>}{locks>0&&<article><span>{locks}</span><div><b>Tension locks</b><small>Two per brace piece</small></div><em className="bom-status">Required</em></article>}{takeoff.accessories.map(a=><article key={a.mappingKey}><span>{a.quantity}</span><div><b>{a.mappingKey.replaceAll("_"," ")}</b><small>Frame hardware</small></div><em className="bom-status">Required</em></article>)}</div>}
