...
import { addBoundarySegment, Design, Heading, historyFor, pointsFor, prepareFrameTakeoffForDesigns, removeBoundarySegment, splitForStock, summaryForDesigns } from "../lib/geometry";
...
export default function Designer() {
  ...
  const takeoff = useMemo(() => prepareFrameTakeoffForDesigns(designs, profile, "configured"), [designs, profile]);
  ...
  return <div className="shell">
    ...
    <main className="layout">
      ...
      <aside className="results">
        ...
        {tab==="cuts"&&<CutList designs={designs} activeSeries={activeSeries} selected={selected} profileLabel={profileLabel} changeLength={changeSeriesLength}/>}
...
function CutList({designs,activeSeries,selected,profileLabel,changeLength}:{designs:Design[];activeSeries:number;selected:string;profileLabel:string;changeLength:(seriesIndex:number,id:string,n:number)=>void}){let n=0;return <div className="data"><h3>Manufacturable pieces</h3><p>Finished outside / long-point measurements.</p>{designs.flatMap((design,seriesIndex)=>design.segments.flatMap(s=>splitForStock(s.length).map((length,j,a)=>({s,length,j,a,seriesIndex})))).map(({s,length,j,a,seriesIndex})=><article key={`${seriesIndex}-${s.id}-${j}`} className={seriesIndex===activeSeries&&selected===s.id?"row-selected":""}><span>{++n}</span><div><b>{profileLabel}</b><small>Series {seriesIndex+1} · {j===0?"45°":"90°"} / {j===a.length-1?"45°":"90°"} end cuts</small></div><label><input type="number" value={length} onChange={e=>changeLength(seriesIndex,s.id,Number(e.target.value))}/><i>mm</i></label></article>)}</div>}
function Bom({takeoff,profileLabel}:{takeoff:ReturnType<typeof prepareFrameTakeoffForDesigns>;profileLabel:string}){return <div className="data"><h3>Bill of materials</h3><p>Calculated directly from all segment series.</p><article><span>{takeoff.cutPieces.length}</span><div><b>{profileLabel}</b><small>{takeoff.profileId}</small></div><em className="bom-status">Required</em></article>{takeoff.accessories.map(a=><article key={a.mappingKey}><span>{a.quantity}</span><div><b>{a.mappingKey.replaceAll("_"," ")}</b><small>Frame hardware</small></div><em className="bom-status">Required</em></article>)}</div>}
