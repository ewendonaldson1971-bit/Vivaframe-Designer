...
import { addBoundarySegment, braceHasSideConnection, createManualBrace, crossBracePiecesForDesigns, Design, Heading, historyFor, pointsFor, prepareFrameTakeoffForDesigns, profileFacingSides, removeBoundarySegment, splitForStock, summaryForDesigns, type BraceConfig, type BracePiece, type Point } from "../lib/geometry";
...
export default function Designer() {
  ...
  const automaticBracePieces = useMemo(()=>automaticBraces?crossBracePiecesForDesigns(designs,braceConfig):[],[automaticBraces,designs,braceConfig]);
  ...
  function removeSelectedManualBrace(){if(!manualBraces.some(piece=>piece.id===selectedBrace))return;setManualBraces(current=>current.filter(piece=>piece.id!==selectedBrace));setBraceOverrides(current=>{const next={...current};delete next[selectedBrace];return next});setSelectedBrace("");setBraceNotice("Manual brace removed.")}
  ...
  function exportDesign(){ const a=document.createElement("a"),blob=new Blob([JSON.stringify({schemaVersion:4,name:"Reception display frame",designs,bracing:{automatic:automaticBraces,manual:manualBraces,overrides:braceOverrides},productIds:{profile}},null,2)],{type:"application/json"});a.href=URL.createObjectURL(blob);a.download="vivaframe-design.json";a.click();URL.revokeObjectURL(a.href); }
  async function importDesign(file?:File){if(!file)return;const data=JSON.parse(await file.text()),imported:Design[]=data.designs??[data.design];setDesigns(imported);setAutomaticBraces(data.bracing?.automatic??true);setManualBraces(data.bracing?.manual??[]);setBraceOverrides(data.bracing?.overrides??{});setActiveSeries(0);setStartPlaced(true);setCanvasOrigin({x:310,y:210});setAdditionHistory([]);setSelectedEnd("end");setSelectedBrace("");setBraceAnchor(null);setProfile(data.productIds?.profile||"vivaframe-ss25");fitDesigns(imported);}

  return <div className="shell">
    <header className="productbar"><div className="brand"><Image src="/vivad-logo.png" alt="Vivad" width={118} height={32} unoptimized priority/><span/><div><h1>VivaFrame Designer</h1><small>Frame configuration</small></div></div><div className="actions"><button onClick={()=>{setDesigns([blankDesign()]);setActiveSeries(0);setStartPlaced(false);setCanvasOrigin({x:310,y:210});setAdditionHistory([]);setSelected("");setSelectedEnd(null);setSelectionNotice("");setDirection(null);setManualBraces([]);setBraceOverrides({});setSelectedBrace("");setBraceAnchor(null);setManualBraceMode(false)}}>＋ New design</button><button onClick={()=>setSaved(true)}>▣ Save design</button><button onClick={exportDesign}>⇩ Export design</button><button onClick={()=>fileRef.current?.click()}>⇧ Import design</button><input ref={fileRef} type="file" accept="application/json" hidden onChange={e=>importDesign(e.target.files?.[0])}/></div></header>
