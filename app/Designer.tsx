...
import { Design, Heading, historyFor, pointsFor, preparePricingTakeoff, splitForStock, summary, type Turn } from "../lib/geometry";
...
export default function Designer() {
  ...
  useEffect(()=>{ const key=(e:KeyboardEvent)=>{const target=e.target as HTMLElement;if(["INPUT","SELECT","TEXTAREA"].includes(target.tagName))return;if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==="z"){e.preventDefault();undo()}else if(e.key==="ArrowLeft")setDirection("W");else if(e.key==="ArrowUp")setDirection("N");else if(e.key==="ArrowRight")setDirection("E");else if(e.key==="ArrowDown")setDirection("S");else if(e.key==="Enter")addSegment();}; window.addEventListener("keydown",key); return()=>window.removeEventListener("keydown",key); });
  ...
  return <div className="shell">
    ...
    <header className="productbar"><div className="brand"><span>V</span><div><b>VIVAD</b><small>VivaFrame Designer</small></div><i>Live</i></div><div className="actions"><button onClick={()=>{setDesign(blankDesign());setStartPlaced(false);setSelected("");setDirection(null)}}>＋ New</button><button onClick={()=>setSaved(true)}>▣ Save</button><button onClick={exportDesign}>⇩ Export</button><button onClick={()=>fileRef.current?.click()}>⇧ Import</button><input ref={fileRef} type="file" accept="application/json" hidden onChange={e=>importDesign(e.target.files?.[0])}/></div></header>
