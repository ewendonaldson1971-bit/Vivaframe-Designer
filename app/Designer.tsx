...
import { addBoundarySegment, Design, Heading, historyFor, pointsFor, preparePricingTakeoff, removeBoundarySegment, splitForStock, summary } from "../lib/geometry";
...
export default function Designer() {
  ...
  return <div className="shell">
    ...
    <header className="productbar"><div className="brand"><span>V</span><div><b>VIVAD</b><small>VivaFrame Designer</small></div><i>Live</i></div><div className="actions"><button onClick={()=>{setDesign(blankDesign());setStartPlaced(false);setCanvasOrigin({x:310,y:210});setAdditionHistory([]);setSelected("");setSelectedEnd(null);setSelectionNotice("");setDirection(null)}}>＋ New</button><button onClick={()=>setSaved(true)}>▣ Save</button><button onClick={exportDesign}>⇩ Export</button><button onClick={()=>fileRef.current?.click()}>⇧ Import</button><input ref={fileRef} type="file" accept="application/json" hidden onChange={e=>importDesign(e.target.files?.[0])}/></div></header>
